  ...
  void detach() {
    ...
    if (isAttached) {
      if (_node.hasPrimaryFocus ||
          (_node._manager != null && _node._manager._nextFocus == _node)) {
...
class FocusManager with DiagnosticableTreeMixin {
  ...
  void _updateHighlightMode() {
    ...
    _lastInteractionWasTouch ??= Platform.isAndroid ||
        Platform.isIOS ||
  ...
  void _handleRawKeyEvent(RawKeyEvent event) {
    ...
    bool handled = false;
    for (FocusNode node in <FocusNode>[
      _primaryFocus,
  ...
  void _willUnfocusNode(FocusNode node) {
    ...
    assert(_focusDebug('Unfocused node $node:', <String>[
      'primary focus is $_primaryFocus',
      'next focus will be $_nextFocus'
    ]));
  }
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(DiagnosticsProperty<FocusNode>('primaryFocus', primaryFocus,
        defaultValue: null));
    properties.add(DiagnosticsProperty<FocusNode>('nextFocus', _nextFocus,
        defaultValue: null));
