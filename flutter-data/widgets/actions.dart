...
class Actions extends InheritedWidget {
  ...
  static ActionDispatcher of(BuildContext context, {bool nullOk = false}) {
    ...
    final InheritedElement inheritedElement =
        context.getElementForInheritedWidgetOfExactType<Actions>();
    final Actions inherited =
  ...
  static bool invoke(
    ...
    assert(() {
      ...
      if (action == null) {
        throw FlutterError(
            'Unable to find an action for an intent in the $Actions widget in the context.\n'
    ...
    return _findDispatcher(actionsElement)
        .invokeAction(action, intent, focusNode: focusNode);
  }

  @override
  bool updateShouldNotify(Actions oldWidget) {
    return oldWidget.dispatcher != dispatcher ||
        !mapEquals<LocalKey, ActionFactory>(oldWidget.actions, actions);
  }
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
        DiagnosticsProperty<Map<LocalKey, ActionFactory>>('actions', actions));
  }
}
...
class _FocusableActionDetectorState extends State<FocusableActionDetector> {
  ...
  void _handleShowHoverHighlight() {
    widget.onShowHoverHighlight
        ?.call(_hovering && widget.enabled && _canShowHighlight);
  }

  void _handleShowFocusHighlight() {
    widget.onShowFocusHighlight
        ?.call(_focused && widget.enabled && _canShowHighlight);
  }
